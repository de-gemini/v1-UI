...
const summary = calculateSummary();
  ...
  // Handler for Step 2 NEXT button
  const handleStep2Next = () => {
    const token = localStorage.getItem("token");
    if (!isTokenValid(token)) {
      // Save form state to localStorage for restoration after login
      const body = {
        selectedType,
        selectedFrequency: frequencyBackendValues[selectedFrequency ?? 0],
        roomCounts,
        selectedAddOns,
        ecoFriendly,
        hooverMop,
  ...
  // On phone number change, update user document if authenticated
  ...
  useEffect(() => {
    if (showAuthModal === false) {
      ...
      if (pending) {
        ...
        setSelectedType(bookingData.selectedType ?? 0);
        setSelectedFrequency(
          bookingData.selectedFrequency
            ? ["weekly", "fortnight", "monthly", "onetime"].indexOf(bookingData.selectedFrequency)
            : null
        );
        setRoomCounts(
          bookingData.roomCounts || roomTypes.map(() => 0)
        );
        setAddress(bookingData.address || "");
        setName(bookingData.name || "");
        setSurname(bookingData.surname || "");
        setPhone(bookingData.phone || "");
        setComments(bookingData.comments || "");
        setDirtLevel(bookingData.dirtLevel || 'medium');
        setEcoFriendly(!!bookingData.ecoFriendly);
        setHooverMop(!!bookingData.hooverMop);
        setDisinfection(!!bookingData.disinfection);
        setErrandHours(bookingData.errandHours || 0);
        setHavePets(!!bookingData.havePets);
        setKeyPickup(!!bookingData.keyPickup);
        ...
        if (details) {
          ...
          if (!surname && d.surname) setSurname(d.surname);
        ...
        <div className="w-full max-w-7xl flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-[350px] bg-yellow-50 rounded-xl shadow-lg p-4 sm:p-6 mt-4 flex flex-col gap-4 lg:sticky lg:top-8 h-fit">
            ...
            <div className="text-sm">
              ...
              <div className="flex justify-between text-xs text-gray-500"><span>Min time price</span><span>£19</span></div>
            </div>
          </div>
        </div>
      )}
